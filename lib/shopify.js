...
//get products by tag
export async function getProductsHandle(){
  try {
    const response = await ShopifyData(productsQuery())
    const products = response.data.products
    return JSON.stringify(products)
  } catch (error) {
    return error
...
export async function collectionHandle(){
  try {
    const response = await ShopifyData(collectionsHandleQuery())
    const colections = response.data.collections
    return JSON.stringify(colections)
  } catch (error) {
    return error
  }
}