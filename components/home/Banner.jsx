...
const Banner =({panerSection})=>{
 ...
 useEffect(()=>{
  ...
  // let newText = text.replace(/Microsoft/g, "W3Schools");
   console.log("sectionHero",_title)
 },[panerSection])
    const settings = {
      dots: true,
      infinite: true,
          ...
          <div key={collection.node.id} className="relative z-10 justify-center " >
            ...
            <div className="absolute z-20 w-fit  md:w-max max-h-fit bg-white p-2  bottom-2 md:inset-y-1/4 right-5 md:inset-x-1/2">
              <div className="border-2 border-gray-400 p-4 md:pt-12 md:pb-6 text-center">
                  <div className="md:px-16 italic text-gray-900 text-lg md:text-3xl font-semibold py-2 md:py-4 capitaliz">{collection.node.title.replace(/banner/g, "")}</div>
                  <div className="md:px-16  text-gray-900 mb-4 hidden md:block">{collection.node.description}</div>
                 <Link href={`/collection/${collection.node.handle}`}><a><div className="bg-gradient-to-tr from-primary to-secondary text-white md:py-4 py-1 md:text-xl text-center w-full md:font-bold">show now</div></a></Link>
              </div>
            </div>
        </div>
        ))
