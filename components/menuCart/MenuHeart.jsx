...
function MenuCart({setMenuHeart}) {
  ...
  return (
        ...
        <motion.div initial={{x:i18n.language==="ar"?-400:400,display:"none"}} animate={{x:0,display:"block"}}  transition={{duration:0.2,type:"just"}} className={`${i18n.language==="ar"?"left-0":"right-0"} z-50 bg-white text-gray-900 fixed bottom-0 w-3/4 md:w-1/6  h-full border border-gray-400 px-8`}>
        <div className="">
        ...
        {lovedProduct.length>0&&<div className="overflow-y-auto overflow-x-hidden h-2/3 scroll-smooth scroll-m-0 scroll-p-0 mt-4">
        {lovedProduct.length>0&&lovedProduct.map(product=>(
            <div key={product.id} className="border border-gray-400 relative  my-2">
